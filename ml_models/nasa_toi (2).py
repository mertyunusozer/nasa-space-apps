...
Original file is located at
    https://colab.research.google.com/drive/10E4NpI_ulzTywAnLjTqJXeepbiZJqWgZ
"""
...
path = "/content/drive/MyDrive/Makine_öğrenmesi/NASA/işlenmiş TOI.csv"
...
ax.set_title("RandomForest Gini Importances (Top 20)")
plt.tight_layout(); plt.show()
...
print("Yeni eklenen özellikler:", [c for c in df_fe.columns if c not in df.columns])
...
perm = permutation_importance(rf_fitted, X_test_tr, y_test, n_repeats=10, random_state=42, n_jobs=-1)
perm_df = pd.DataFrame({'feature': feature_names_out, 'importance': perm.importances_mean}) \
            .sort_values('importance', ascending=False).head(20)

perm_df.plot.barh(x='feature', y='importance', figsize=(8,6), legend=False)
plt.title("Permutation Importances (Top 20)")
plt.show()
# train_model.py (PyCharm'da)
import joblib

# ... X, y, pipeline fit ...
# pipeline: scikit-learn Pipeline veya benzeri (fit edilmiş)
joblib.dump(pipeline, "model/pipeline.pkl")  # projenin kökünde /model klasörü
print("Saved to model/pipeline.pkl")
