...
Original file is located at
    https://colab.research.google.com/drive/1t-odJUVjfNYYGczM7hP9Bhoj7CbKt_hs
"""
...
df0 = pd.read_csv(PATH, comment="#", sep=None, engine="python", encoding="utf-8")
df0.columns = (df0.columns.str.strip()
                        .str.replace(r"\s+","_", regex=True)
                        .str.lower())
m_k2 = df0.get("disc_facility","").astype(str).str.contains("k2", case=False, na=False)
m_tr = df0.get("discoverymethod","").astype(str).str.contains("transit", case=False, na=False)
df0 = df0[m_k2 & m_tr] if (m_k2.any() and m_tr.any()) else df0
...
y = (df0[label_col].astype(str).str.upper()
       .map({"CONFIRMED":1, "FALSE POSITIVE":0}))
df = df0[y.isin([0,1])].copy()
y = y[y.isin([0,1])].astype(int)
print("Satır:", len(df), "| Pozitif oranı:", y.mean().round(3))
...
if group_col: hard_drop.append(group_col)
...
for c in num_cols:
    v = X[c].values
    m = ~np.isnan(v)
    if m.sum() >= 30 and y[m].nunique()==2:
        try:
            auc = roc_auc_score(y[m], v[m])
            if auc >= 0.98 or auc <= 0.02:
                leaky_single.append((c, auc))
        except Exception:
            pass

print("Tek başına AUC≥0.98 veren şüpheli kolonlar (sadece rapor):")
for c, auc in sorted(leaky_single, key=lambda x:-abs(x[1]-0.5)):
...
if group_col:
    ...
    scores_gkf = cross_val_score(pipe, X_num, y, cv=gkf.split(X_num, y, groups), scoring="roc_auc")
    print("GroupKFold ROC-AUC:", np.round(scores_gkf, 3), "mean=", scores_gkf.mean().round(3))

X_work = X_num.copy()
y_work = y.copy()
groups_work = df[group_col] if group_col else None
...
gkf = GroupKFold(n_splits=5) if group_col else KFold(n_splits=5, shuffle=True, random_state=42)

y_true_all, y_pred_all, y_prob_all = [], [], []
for tr, te in gkf.split(X_work, y_work, groups_work):
...
leak_suspects = [c for c in X_work.columns
                 if any(p in c.lower() for p in [
                     "sy_pnum", "pnum",
                     "err", "error", "dist", "radeerr",
                     "st_teff", "st_rad", "st_rader", "st_mass"
                 ])]

print("Şüpheli leak kolonları:", leak_suspects)
...
gkf = GroupKFold(n_splits=5)
scores = []
for tr, te in gkf.split(X_clean, y, df[group_col]):
    pipe.fit(X_clean.iloc[tr], y.iloc[tr])
    prob = pipe.predict_proba(X_clean.iloc[te])[:,1]
